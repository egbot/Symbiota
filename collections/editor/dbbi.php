...
if($SYMB_UID){
	...
	if($isEditor == 1 || $isEditor == 2){
		...
		if($occManager->remapImage($_POST["imgid"], $_POST["targetoccid"])){
                    // changed to dbbi
		    $statusStr = 'SUCCESS: Image remapped to record <a href="dbbi.php?occid='.$_POST["targetoccid"].'" target="_blank">'.$_POST["targetoccid"].'</a>';
                    if($SOLR_MODE) $solrManager->updateSOLR();
		}
		else{
	...
	if($qryCnt == 0){
	    ...
	    $navStr = '<b>';
	    if($occIndex > 0) $navStr .= '<a href="dbbi.php?csmode='.($crowdSourceMode).'&occindex='.(0).'&collid='.($collId).'" title="First Record">';
	    $navStr .= '|&lt;';
	    if($occIndex > 0) $navStr .= '</a>';
	    $navStr .= '&nbsp;&nbsp;&nbsp;&nbsp;';
	    if($occIndex > 0) $navStr .= '<a href="dbbi.php?csmode='.($crowdSourceMode).'&occindex='.($occIndex-1).'&collid='.($collId).'" title="Previous Record">';
	    $navStr .= '&lt;&lt;';
	    if($occIndex > 0) $navStr .= '</a>';
	    $recIndex = ($occIndex<$qryCnt?($occIndex + 1):'*');
	    $navStr .= '&nbsp;&nbsp;| '.$recIndex.' of '.$qryCnt.' |&nbsp;&nbsp;';
	    if($occIndex<$qryCnt-1) {
                if ($statusStr == "SUCCESS: edits submitted and activated ") {
                    $navStr .= '<a id="nextrecord" href="dbbi.php?csmode='.($crowdSourceMode).'&occindex='.($occIndex).'&collid='.($collId).'" title="Next Record">';
                }
                else {
                    $navStr .= '<a id="nextrecord" href="dbbi.php?csmode='.($crowdSourceMode).'&occindex='.($occIndex+1).'&collid='.($collId).'" title="Next Record">';
                }
		echo '<link ref="prefetch" href="dbbi.php?csmode='.($crowdSourceMode).'&occIndex='.($occIndex+1).'&collid='.($collId).'"></link>';
            }
	    $navStr .= '&gt;&gt;';
	    if($occIndex<$qryCnt-1) $navStr .= '</a>';
	    $navStr .= '&nbsp;&nbsp;&nbsp;&nbsp;';
	    if($occIndex<$qryCnt-1) $navStr .= '<a href="dbbi.php?csmode='.($crowdSourceMode).'&occindex='.($qryCnt-1).'&collid='.($collId).'" title="Last Record">';
	    $navStr .= '&gt;|';
	    if($occIndex<$qryCnt-1) $navStr .= '</a> ';
	    if(!$crowdSourceMode){
		$navStr .= '&nbsp;&nbsp;&nbsp;&nbsp;';
...
<html>
	...
	<title><?php echo $DEFAULT_TITLE; ?> DB By IMG</title>
	...
	?>
	    <link href="../../css/dbbi.css?ver=170601" type="text/css" rel="stylesheet" id="editorCssLink" />
	    <?php
	    if(isset($CSSARR)){
	    ...
	    <script src="../../js/jquery.imgareaselect.js?ver=140310" type="text/javascript"></script>
	    ...
	    <script type="text/javascript" src="../../js/symb/collections.dbbimain.js?ver=20170503"></script>
	    ...
	    <script type="text/javascript" src="../../js/symb/collections.dbbishare.js?ver=170302"></script>
    </head>
    <body>
	<!-- inner text -->
	<div id="innertext">
		...
		<div id="navDiv">
			...
			<div class='navpath'>
			    ...
			    ?>
			    <b><?php if($isEditor == 3) echo 'Taxonomic '; ?>DB By IMG</b>
			</div>
		    <?php
		    }
		    ?>
		...
		?>
			...
			<b>Action Status: </b>
			<span id="actionStatus" style="color:<?php echo (stripos($statusStr,'ERROR')!==false?'red':'green'); ?>;"><?php echo $statusStr; ?></span>
			<?php
			if($action == 'Delete Occurrence'){
			?>
			    <br/>
		...
		?>
			...
			<tr><td id="editortd" style="" valign="top">
				...
				<fieldset>
				    <legend style="font-weight:bold;">Edit Table</legend>
				    <?php
				    if($occId && $isEditor){
				    ...
				    <div id="occdiv">
					<form id="fullform" name="fullform" action="dbbi.php" method="post" onsubmit="return verifyFullForm(this);">
					    <fieldset>
						<?php
						if($occId){
						...
						<div style="clear:both;">
							...
							<br/>
							<input id="recordedByInput" type="text" name="recordedby" tabindex="6" maxlength="255" value="<?php echo array_key_exists('recordedby',$occArr)?$occArr['recordedby']:''; ?>" onchange="fieldChanged('recordedby');" />
						    </div>
						...
						<div style="clear:both;">
							...
							<br/>
							<input type="text" name="scientificnameauthorship" maxlength="100" tabindex="30" value="<?php echo array_key_exists('scientificnameauthorship',$occArr)?$occArr['scientificnameauthorship']:''; ?>" onchange="fieldChanged('scientificnameauthorship');" <?php if(!$isEditor || $isEditor == 3) echo 'disabled'; ?> />
						    </div>
						...
						<div id="localityDiv">
						    ...
						    <a href =" #" id="localityExtraToggle" onclick="toggle('localityExtraDiv');">
							<img src="../../images/editplus.png" style="width:15px;" />
						    </a>
						</div>
						<?php
						...
						<div id="locsecreason" style="margin-left:5px;border:2px solid gray;display:inline-block;display:<?php echo ($lsrValue?'inline':'none') ?>;padding:3px">
						    <div style="display:inline-block"><input name="lockLocalitySecurity" type="checkbox" onchange="securityLockChanged(this)"  <?php echo ($lsrValue?'checked':'') ?> /> Lock Security Setting</div>
						    <?php 
						...
						<input id="occid" type="hidden" name="occid" value="<?php echo $occId; ?>" />
	    ...
	    ?>
			...
			<td id="imgtd" style="display:none;width:430px;" valign="top">
				...
				include_once('includes/dbbiprocessor.php');
			    }
			    ?>
			</td>
			<script type="text/javascript">
			 // autofocus selected query field
			 if (document.getElementsByName(document.queryform.elements.namedItem("q_customfield1").selectedOptions.item(0).value.toLowerCase()).item(0) != null) {
                             var customField1 = document.getElementsByName(document.queryform.elements.namedItem("q_customfield1").selectedOptions.item(0).value.toLowerCase()).item(0);
			     if (customField1 != null) customField1.focus();
			     // cookie auto-click next record
                             if (String(/submit=[0-9]/.exec(document.cookie)).replace("submit=","") == "1") {
				 document.cookie = "submit=" + 0 + "; max-age=" + 3600;
				 // if it's submit, grab the submit value and throw it in a cookie
				 var savedValue = document.getElementsByName(customField1.name)[0].value;
				 var fieldName = document.getElementsByName(customField1.name)[0].name;
				 if (savedValue != "") {
				     document.cookie = fieldName + "=" + savedValue + "; max-age=" + 3600;
				 }
				 document.getElementById("nextrecord").click();
                             }
			     else {
				 // if not, throw the cookie value into the selected query field
				 // but, only if the query field is empty
				 var customField1 = document.getElementsByName(document.queryform.elements.namedItem("q_customfield1").selectedOptions.item(0).value.toLowerCase()).item(0);
				 var fieldName = customField1.name;
				 var reField = fieldName+"=([^;]+)";
				 var regex = new RegExp(reField, "g");
				 if (customField1.value == "") {
				     var savedValue = regex.exec(document.cookie);
				     if (savedValue != null) {
					 customField1.value = savedValue[1];
					 customField1.onchange();
					 customField1.select();
				     }
				 }
			     }
			 }
			 // add thumbnail to edit table for reference
			 var thumbnail = new Image(100,150);
			 thumbnail.src = document.getElementById("activeimg-1").src;
			 thumbnail.style = "float:right";
			 var preElement = document.getElementById("fflocality");
			 preElement.insertAdjacentElement('afterend', thumbnail);
                        </script>
                        <script type="text/javascript">
			 (function($) {
			     // code taken from jquery imagetool
			     // ensures the image is displayed correctly before and after zooming
			     $.widget("ui.imagetool", {
				 options: {
				     src: null
				     ,viewportWidth: 550
			 ...
			 // selectZoom runs after user has drawn selection area on the image
			 function selectZoom (img, selection){
			     // reset image params to default on click when the image is "zoomed"
			     if (Number(img.style.width.replace(/[a-z]+/, "")) > 550) {
				 $('img#activeimg-1').css({
				     width: 366.667 + 'px',
				     height: 500 + 'px',
				     marginLeft: 91.667 + 'px',
				     marginTop: 0 + 'px' });
			     }
			     // save selection to session cookie
			     // and zoom to selection area
			     else {
				 // set cookie values
				 document.cookie = "width=" + selection.width + "; max-age=" + 3600;
				 document.cookie = "height=" + selection.height + "; max-age=" + 3600;
				 document.cookie = "x1=" + selection.x1 + "; max-age=" + 3600;
				 document.cookie = "y1=" + selection.y1 + "; max-age=" + 3600;
				 // scale up by 1.5 (ratio of viewport vs. image)
				 var scaleX = (366.667 / (selection.width)) * 1.5;
				 var scaleY = 500 / (selection.height);
				 $('img#activeimg-1').css({
				     width: Math.round(scaleX * 366.667) + 'px',
				     height: Math.round(scaleY * 500) + 'px',
				     marginLeft: '-' + Math.round(scaleX * selection.x1) + 'px',
				     marginTop: '-' + Math.round(scaleY * selection.y1) + 'px' });
			     }
			 }
			 // cZoom runs on imgareaselect tool's init
			 function cZoom (img, selection) {
			     // zoom to last selection area params if cookie has selection params
			     if ((/width=[0-9]+/.exec(document.cookie) != null) && (/height=[0-9]+/.exec(document.cookie) != null) && (/x1=[0-9]+/.exec(document.cookie) != null) && (/y1=[0-9]+/.exec(document.cookie) != null)) {
				 var scaleCX = (366.667 / Number(String(/width=[0-9]+/.exec(document.cookie)).replace("width=", ""))) * 1.5;
				 var scaleCY = 500 / Number(String(/height=[0-9]+/.exec(document.cookie)).replace("height=", ""));
				 var Cx1 = Number(String(/x1=[0-9]+/.exec(document.cookie)).replace("x1=", ""));
				 var Cy1 = Number(String(/y1=[0-9]+/.exec(document.cookie)).replace("y1=", ""));
				 
				 $('img#activeimg-1').css({
				     width: Math.round(scaleCX * 366.667) + 'px',
				     height: Math.round(scaleCY * 500) + 'px',
				     marginLeft: '-' + Math.round(scaleCX * Cx1) + 'px',
				     marginTop: '-' + Math.round(scaleCY * Cy1) + 'px' });
			     }
			     // set image to default values
			     else {
				 $('img#activeimg-1').css({
				     width: 366.667 + 'px',
				     height: 500 + 'px',
			 ...
			 $(document).ready(function () {
			     // hide ocr button and options
			     labelProcDiv = document.getElementById("labelprocessingdiv");
			     for (var i = 0; i < labelProcDiv.children.length; i++) {
				 var currentElement = labelProcDiv.children[i];
				 var elementAttributes = currentElement.attributes;
				 for (var j = 0; j < currentElement.children.length; j++) {
				     currentElementChildren = currentElement.children[j];
				     for (var k = 0; k < currentElementChildren.children.length; k++) {
					 childrenOfChildren = currentElementChildren.children;
					 childAttributes = currentElementChildren.children[k].attributes;
					 for (var l = 0; l < childAttributes.length; l++) {
					     // hide OCR button
					     if ((childAttributes[l].name == "value") && (childAttributes[l].value == "OCR Image")){
						 childrenOfChildren[k].style.display = "none";
					     }
					     // hide OCR options box
					     else if ((childAttributes[l].name == "style") && (childAttributes[l].value == "width:200px;background-color:lightyellow;")) {
						 childrenOfChildren[k].style.display = "none";
					     }
					 }
				     }
			     ...
			     // end of hide ocr button code block

			     // enable imgAreaSelect functions on activeimg-1
			     $('#activeimg-1').imgAreaSelect({ onInit: cZoom, autoHide: true, onSelectEnd: selectZoom, enable: true, show: true, movable: true, resizable: false, autoHide: true });
			 });
			 // end of js
			</script>
			</tr>
